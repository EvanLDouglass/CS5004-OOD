...
public class Swimmer implements Comparable<Swimmer> {
  ...
  /**
   * Get's the average 50m freestyle time for this swimmer.
   *
   * @return the average freestyle time or -1.0 if the swimmer has no freestyle times.
   */
  public Double getAvgFreestyleTimes() {
    if (freestyle50mTimes.size() == 0) {
      return -1.0;
    }
    ...
    return total / freestyle50mTimes.size();
  }

  @Override
  //YOUR CODE HERE
  public int compareTo(Swimmer otherSwimmer) {
    if (getAvgFreestyleTimes() < otherSwimmer.getAvgFreestyleTimes()) {
      return -1;
    } else if (getAvgFreestyleTimes() > otherSwimmer.getAvgFreestyleTimes()) {
      return 1;
    } else {
      return 0;
    }
